...
import {
  ...
  kpis, entities, alerts, emergingNetworks, listings, wallets,
          ...
          <tbody>
            {entities.map(e=>(
              <tr key={e.id} onClick={()=>navigate("entity",e)}>
                ...
                <td>{e.confidence}%</td>
                <td><span style={{fontSize:11,color:"var(--text-3)"}}>{e.marketplaces}M · {e.wallets}W · {e.listings}L · {e.comms}C</span></td>
                <td>{e.firstSeen}</td>
                <td>{e.lastSeen}</td>
...
function ListingsScreen() {
  const [sel, setSel] = useState<typeof listings[0]|null>(null);
  return (
    <div style={{padding:"26px 28px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
        <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:sel?"1fr 360px":"1fr",gap:16,transition:"all 0.25s"}}>
        <div className="card">
          <table className="data-table">
            <thead><tr><th>Record ID</th><th>Source</th><th>Category</th><th>Risk</th><th>Signals</th><th>First Seen</th><th>Last Seen</th><th>Status</th></tr></thead>
            <tbody>
              {listings.map(l=>(
                <tr key={l.id} className={sel?.id===l.id?"selected":""} onClick={()=>setSel(l.id===sel?.id?null:l)}>
                  <td><span className="mono" style={{color:"var(--accent-hi)",fontSize:12}}>{l.id}</span></td>
                  <td>{l.source}</td>
                  ...
                  <td>{l.last}</td>
                  <td>
                    <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:l.status==="Flagged"?riskBg(80):l.status==="Under Review"?riskBg(60):l.status==="Monitoring"?riskBg(40):"rgba(255,255,255,0.04)",color:l.status==="Flagged"?riskColorLight(80):l.status==="Under Review"?riskColorLight(60):l.status==="Monitoring"?riskColorLight(40):"var(--text-3)",border:`1px solid ${l.status==="Flagged"?riskBorder(80):l.status==="Under Review"?riskBorder(60):l.status==="Monitoring"?riskBorder(40):"var(--border)"}`}}>{l.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {sel && (
          <div className="card anim-slide-r" style={{padding:20,height:"fit-content"}}>
            ...
            {[
              {l:"Source",v:sel.source},{l:"Category",v:sel.category},
              {l:"Risk Score",v:`${sel.risk} / 100`},{l:"Signals",v:`${sel.signals} detected`},
              {l:"First Seen",v:sel.first},{l:"Last Seen",v:sel.last},{l:"Status",v:sel.status},
            ].map(item=>(
