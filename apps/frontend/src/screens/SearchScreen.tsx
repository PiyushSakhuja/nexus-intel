...
type ResultKind = "Entities" | "Wallets" | "Listings" | "Investigations";
...
export function SearchScreen({ navigate }: { navigate:(s:string,d?:any)=>void }) {
  ...
  const chips: ("All"|ResultKind)[] = ["All","Entities","Wallets","Listings","Investigations"];
  ...
  return (
    <div style={{padding:"26px 28px"}}>
      <div style={{marginBottom:22}}>
        ...
        <p className="page-sub">Search across entities, wallets, listings, and investigations.</p>
      </div>
      ...
      {!searched && !error && (
        <div style={{textAlign:"center",padding:"70px 0",color:"var(--text-4)"}}>
          ...
          <div style={{fontSize:12,marginTop:6,color:"var(--text-4)"}}>Searches across entities, wallets, listings, and investigations</div>
        </div>
      )}
                ...
                <div key={e.id} className={`card card-hover anim-fade-up delay-${Math.min(i+1,5)}`} style={{padding:"18px 20px"}}>
                  <div style={{display:"flex",gap:18,alignItems:"flex-start"}}>
                    ...
                    <div style={{display:"flex",gap:7,flexShrink:0}}>
                      <button className="btn btn-primary btn-sm" onClick={()=>navigate("entity",{ id: e.displayId, displayId: e.displayId })}>View Entity</button>
              ...
              {showListings && results.listings.map((l:any)=>(
                <div key={l.id} className="card card-hover" style={{padding:"18px 20px"}}>
                  <div style={{display:"flex",gap:18,alignItems:"flex-start"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                        <span style={{fontSize:9,color:"var(--medium-light)",textTransform:"uppercase",letterSpacing:"0.1em",background:"rgba(217,119,6,0.1)",padding:"2px 7px",borderRadius:3,fontWeight:600}}>LISTING</span>
                        <span style={{fontSize:14,fontWeight:600,color:"var(--text-1)"}}>{l.title}</span>
                      </div>
                      <div style={{fontSize:12,color:"var(--text-3)"}}>Vendor: <span style={{color:"var(--text-2)"}}>{l.vendorAlias}</span> · {l.marketplace}</div>
                    </div>
                    <button className="btn btn-ghost btn-sm" onClick={()=>navigate("listings")}>View Listing</button>
                  </div>
                </div>
              ))}
